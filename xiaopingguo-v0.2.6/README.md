...
本版修复持久化数据库的关键问题：生产环境不再静默回退到 `/tmp` SQLite；启动时对 CloudBase MySQL 做真实写入+读回自检；“录入完成”后必须从 MySQL 重新读回同一份原文和版本才会宣布成功；`数据库状态` 返回实时表计数；私聊数据库异常会直接显示，不再只写日志。
...
`小苹果 v0.2.6｜数据库：CloudBase MySQL HTTP API（持久化）｜环境：...｜实时档案 N｜管理员 N｜草稿 N`
