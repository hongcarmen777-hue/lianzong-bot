...
这一版的核心变化只有一个：**正式数据可以通过 CloudBase MySQL HTTP API 持久化，不需要给现有云托管服务配置 VPC，也不需要数据库用户名/密码。**

小苹果的产品定位仍然保持极简：

- 骰主把历届恋综原文直接录给它；原文完整保存，同时整理出 P 编号、标题、TAG、日程、身份牌等结构化索引。
...
如果你的 CloudBase 里还写着旧的 `deepseek-flash`，v0.2.3 会自动转换成 `deepseek-v4-flash`，但建议以后直接改成正式 ID。
...
`小苹果 v0.2.3｜数据库：CloudBase MySQL HTTP API（持久化）`
