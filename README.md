...
v0.2.2 同时支持 MySQL 和 PostgreSQL。推荐在 CloudBase MySQL 开启直连后，把下面参数加入云托管环境变量：
...
旧的 v0.1 数据表即使还在 SQLite / SQL 数据库中也不会被 v0.2.2 使用。
...
认主后的骰主私聊不再反复提示“认主”。程序能力问题会直接回答，其他私聊可作为调试/自然聊天入口；正式档案录入仍放在唯一主群。
